...
import { MdDashboard, MdHistory, MdPostAdd } from 'react-icons/md';
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (!snapshot.exists()) {
        ...
        setUserInfo({ ...userData, ...defaultProfile });
        setEditingData(defaultProfile);
      } else {
            ...
            <div className="userCard">
              ...
              <div className="actionButtons">
                <button onClick={toggleModal} className="glassbutton">
                  ...
                  <span className="label">{t('common.editProfile')}</span>
                </button>

                <button className="glassbutton" onClick={() => router.push(`/${lang}/articles/new`)}>
                  <MdPostAdd className="icon" />
                  <span className="label">{t('article.create')}</span>
                </button>

                <button className="glassbutton" onClick={() => router.push(`/${lang}/profile/manage`)}>
                  <MdHistory className="icon" />
                  <span className="label">{t('article.history')}</span>
                </button>

                {isAdmin && (
                  <button className="glassbutton" onClick={() => router.push(`/${lang}/dashboard`)}>
